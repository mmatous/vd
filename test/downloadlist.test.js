...
describe('DownloadList class', () => {
	...
	describe('getByDigestId()', () => {
		test('retrieves correct entry', () => {
			const list = new DownloadList(2);
	...
	describe('hasRegularDownload()', () => {
		test('returns true if RD exists', () => {
			const list = new DownloadList(2);
			list.createEntry(helpers.testDownloadItem);
		...
		test('returns false if RD does not exist', () => {
			const list = new DownloadList(2);
			list.createEntry(helpers.testDownloadItem);
	...
	describe('hasDigest()', () => {
		test('returns true if digest exists', () => {
			const list = new DownloadList(2);
			const res = list.createEntry(helpers.testDownloadItem);
			res.setDigestFile(helpers.testDigestItem);
		...
		test('returns false if digest does not exist', () => {
			const list = new DownloadList(2);
			list.createEntry(helpers.testDownloadItem);
...
describe('DownloadListItem class', () => {
	describe('markDownloaded()', () => {
		test('marks file downloaded if passed file ID', () => {
		...
		test('marks digest if passed digest ID', () => {
			const entry = new DownloadListItem(helpers.testDownloadItem);
		...
		test('marks signature if passed signature ID', () => {
			const entry = new DownloadListItem(helpers.testDownloadItem);
		...
		test('throws if provided with invalid ID', () => {
			const entry = new DownloadListItem(helpers.testDownloadItem);
	...
	describe('readyForVerification()', () => {
		test('returns true if both file and digest finished downloading', () => {
			const entry = new DownloadListItem(helpers.testDownloadItem);
			entry.setDigestFile(helpers.testDigestItem);
		...
		test('returns true if file finished and digest is set manually', () => {
			const entry = new DownloadListItem(helpers.testDownloadItem);
		...
		test('returns true if both file and signature finished downloading', () => {
			const entry = new DownloadListItem(helpers.testDownloadItem);
			entry.setSignatureFile(helpers.testSigItem);
	...
	describe('serialize()', () => {
		test('returns object for consumption by vd-verifier (digestFile)', () => {
			const entry = new DownloadListItem(helpers.testDownloadItem);
			entry.setDigestFile(helpers.testDigestItem);
		...
		test('returns object for consumption by vd-verifier (hexStr digest)', () => {
			const entry = new DownloadListItem(helpers.testDownloadItem);
			const hex = '277c1bfe069a889eb752d3c630db34310102b2bb2f0c0ff11cf4246e333b3503';
			entry.setDigest(hex);
			const res = entry.serialize();
		...
		test('returns object for consumption by vd-verifier (detached signature)', () => {
			const entry = new DownloadListItem(helpers.testDownloadItem);
			entry.setSignatureFile(helpers.testSigItem);
			const res = entry.serialize();
		...
		test('throws if no field for verification present', () => {
			const entry = new DownloadListItem(helpers.testDownloadItem);
